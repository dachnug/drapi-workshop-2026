...
export class DnugLogin extends LitElement {
  ...
  /**
   * Starts the OIDC (PKCE) login flow.
   *
   * Captures the current page URL as the post-login redirect target, asks
   * {@link oidcLogin} for the IdP authorization URL (using
   * {@link OIDC_DISCOVERY_URL}), and navigates the browser there.
   */
    ...
    return html`
      <wa-dialog label="Login">
        <form @submit=${this.handleSubmit}>
          <wa-input name="username" label="Username" autocomplete="username" required></wa-input>
          <wa-input
            name="password"
            label="Password"
            type="password"
