...
import { extractCredentials, getCurrentUser, isLoggedIn, keepFetch, loginBasic, logout, parseJwtClaims } from '../src/auth';
...
describe('refresh token flow', () => {
  it('refreshes a token with the stored refreshToken, then proceeds with the request', async () => {
    const newToken = makeToken({ CN: 'Alice', exp: futureExp() });
    globalThis.localStorage.setItem('refreshToken', 'rt');
    const fetchSpy = vi
      .spyOn(globalThis, 'fetch')
      .mockResolvedValueOnce(makeResponse({ bearer: newToken }))
      .mockResolvedValueOnce(makeResponse({ ok: true }));

    const result = await keepFetch('/api/v1/things');

    expect(result).toEqual({ ok: true });
    // First call is the refresh POST to the fallback endpoint with the grant body.
    const [refreshUrl, refreshInit] = fetchSpy.mock.calls[0] as [string, RequestInit];
    expect(refreshUrl).toBe('/oauth/token');
    expect(refreshInit.method).toBe('POST');
    expect(String(refreshInit.body)).toContain('grant_type=refresh_token');
    // Second call is the actual request carrying the freshly refreshed token.
    const requestInit = fetchSpy.mock.calls[1][1] as RequestInit;
    expect(new Headers(requestInit.headers).get('Authorization')).toBe(`Bearer ${newToken}`);
  });

  it('refreshes against the stored token_endpoint when present', async () => {
    const newToken = makeToken({ exp: futureExp() });
    globalThis.localStorage.setItem('refreshToken', 'rt');
    ...
    await keepFetch('/api/v1/things');

    expect(fetchSpy.mock.calls[0][0]).toBe('https://idp/token');
  });

  it('removes the refreshToken and rejects when the refresh request is not ok', async () => {
    globalThis.localStorage.setItem('refreshToken', 'rt');
    vi.spyOn(globalThis, 'fetch').mockResolvedValue(makeResponse(null, { status: 500 }));

    await expect(keepFetch('/api/v1/things')).rejects.toThrow('Token refresh failed: 500');
    expect(globalThis.localStorage.getItem('refreshToken')).toBeNull();
  });

  it('does not reprompt after a failed refresh, even when reprompt is true', async () => {
    globalThis.localStorage.setItem('refreshToken', 'rt');
    const dialog = installLoginDialog(() => simulateLoginSuccess(makeToken({ exp: futureExp() })));
    vi.spyOn(globalThis, 'fetch').mockResolvedValue(makeResponse(null, { status: 500 }));

    await expect(keepFetch('/api/v1/things', {}, true)).rejects.toThrow('Token refresh failed: 500');
    expect(dialog.showCalls).toBe(0);
  });
});

describe('extractCredentials', () => {
  it('accepts access_token, resolves the username, and persists the refresh_token', () => {
    const token = makeToken({ CN: 'Al', exp: futureExp() });
    const authChanged = vi.fn();
    globalThis.addEventListener('auth-changed', authChanged);

    extractCredentials({ access_token: token, refresh_token: 'r1' });

    expect(globalThis.localStorage.getItem('authToken')).toBe(token);
    expect(globalThis.localStorage.getItem('username')).toBe('Al');
    expect(globalThis.localStorage.getItem('refreshToken')).toBe('r1');
    expect(authChanged).toHaveBeenCalledTimes(1);

    globalThis.removeEventListener('auth-changed', authChanged);
  });

  it('throws when neither bearer nor access_token is present', () => {
    expect(() => extractCredentials({})).toThrow('bearer token missing');
  });
});
